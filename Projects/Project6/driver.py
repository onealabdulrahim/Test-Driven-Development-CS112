...
while ctr < len(testCases):
	...
	if pieces[0] == "Number.__init__":	
		...
		try:				
			...
			result = "EXCEPTION"
	elif pieces[0] == "Banner.__init__":
		banner = Banner(pieces[1])
		if pieces[2] == "Banner.__str__":
			result = str(banner)
	elif pieces[0] == "Banner.addNumber":
		banner.addNumber(number,pieces[2])
		if pieces[3] == "Banner.__str__":
			result = str(banner)
	elif pieces[0] == "main":
		try:
